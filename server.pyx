...
import json
import time
# Use Python's int() and float(), and Cython's cfloat and cint for C-level types
from cython cimport float as cfloat, int as cint

# We declare the types to enhance performance
address = os.getenv('ADDRESS', 'localhost')
port = int(os.getenv('PORT', 5135))

# Statically typing the controller dictionary
cdef dict controllers = {
	1: moteus.Controller(id=1),
	2: moteus.Controller(id=2),
}

cdef dict last_poses = {cid: None for cid in controllers}
...
async def handle_client(r, w):
	...
	try:
		...
		# Use Python's native variable types in async function
...
# Keep this function optimized with cdef as it's not async
cdef dict parse_commands(str data):
	cdef dict default_commands = {
		"id": None,
		"p": 0.0,
		"d": True,
	}
	cdef dict commands = {}  # This needs to be declared here, outside the try block
	cdef list json_data
	cdef int motor_id
	cdef dict item

	try:
		json_data = json.loads(data)  # This line is inside the try block now
			...
			if motor_id is not None:
				...
				for key, value in item.items():
					if key in default_commands:
						# Use Python's int() and float(), as isinstance() checks are at the Python level
...
async def control_loop():
	...
	while True:
		...
		await asyncio.sleep(0.0)  # Yield control to the event loop
